...
function Kpi({ icon, label, value, tone }: { icon: string; label: string; value: number; tone: string }) {
  return <div className="role-kpi"><span className={`role-kpi-icon ${tone}`}><Icon name={icon} size={24} /></span><div><small>{label}</small><strong>{Number(value || 0).toLocaleString()}</strong><p className="up">Live <em>current data</em></p></div></div>;
}
...
function SupplyChainDashboard({ data }: { data: any }) {
  const [tab,setTab]=useState<TabKey>("po"), [from,setFrom]=useState(""), [to,setTo]=useState(""), [vendor,setVendor]=useState(""), [status,setStatus]=useState(""), [search,setSearch]=useState("");
  const [applied,setApplied]=useState({from:"",to:"",vendor:"",status:"",search:""}); const [page,setPage]=useState(1); const pageSize=10;
  const source:any[]=tab==="po"?(data.purchaseOrders||[]):tab==="grn"?(data.grns||[]):tab==="pending"?(data.pendingVendorRequests||[]):(data.recentlyOnboardedVendors||[]);
  const vendors=useMemo(()=>Array.from(new Set(source.map((r:any)=>r.vendorName).filter(Boolean))).sort() as string[],[source]);
  const statuses=useMemo(()=>Array.from(new Set(source.map((r:any)=>r.status||r.accessStatus||(r.active?"Active":"Inactive")).filter(Boolean))).sort() as string[],[source]);
  const filtered=useMemo(()=>source.filter((r:any)=>{const date=r.poDate||r.grnDate||r.onboardedAt||r.lastAccess; const rowStatus=r.status||r.accessStatus||(r.active?"Active":"Inactive"); const q=applied.search.toLowerCase(); const text=Object.values(r).join(" ").toLowerCase(); return inRange(date,applied.from,applied.to)&&(!applied.vendor||r.vendorName===applied.vendor)&&(!applied.status||rowStatus===applied.status)&&(!q||text.includes(q));}),[source,applied]);
  const pages=Math.max(1,Math.ceil(filtered.length/pageSize)),safePage=Math.min(page,pages),visible=filtered.slice((safePage-1)*pageSize,safePage*pageSize); useEffect(()=>setPage(1),[tab,applied]);
  const a=data.accessStatus||{}, total=Number(a.total||0),enabled=Number(a.portalEnabled||0),notEnabled=Number(a.notEnabled||0); const donutStyle={background:`conic-gradient(#16a34a 0 ${total?enabled/total*100:0}%, #d1d5db 0 100%)`};
  const exportRows=()=>{if(tab==="po")exportRowsToCsv("supply-chain-purchase-orders.csv",["PO Number","Vendor","PO Date","Amount (PKR)","Status","GRN Status","Invoice Status"],filtered.map((r:any)=>[r.poNumber,r.vendorName,formatDate(r.poDate),r.amount,r.status,r.grnStatus,r.invoiceStatus])); else if(tab==="grn")exportRowsToCsv("supply-chain-grns.csv",["GRN Number","Vendor","PO Number","GRN Date","Status","QC Status"],filtered.map((r:any)=>[r.grnNumber,r.vendorName,r.poNumber,formatDate(r.grnDate),r.status,r.qcStatus])); else exportRowsToCsv(tab==="pending"?"pending-vendor-access.csv":"recently-onboarded-vendors.csv",["Vendor","Oracle Vendor ID","Status","Last Access"],filtered.map((r:any)=>[r.vendorName,r.oracleVendorId||"",r.accessStatus||(r.active?"Active":"Inactive"),formatDate(r.lastAccess||r.onboardedAt)]));};
  return <div className="role-dashboard supply-dashboard"><section className="role-kpis"><Kpi icon="user" label="Total Vendors" value={data.kpis?.totalVendors} tone="purple"/><Kpi icon="invoice" label="Active Portal Users" value={data.kpis?.activePortalUsers} tone="purple"/><Kpi icon="po" label="Purchase Orders" value={data.kpis?.purchaseOrders} tone="orange"/><Kpi icon="grn" label="GRNs Received" value={data.kpis?.grnsReceived} tone="green"/><Kpi icon="support" label="Open Vendor Requests" value={data.kpis?.openVendorRequests} tone="red"/></section>
    <section className="role-analytics-grid"><div className="role-panel"><h3>Vendor Access Status</h3><div className="role-donut-wrap"><div className="role-donut" style={donutStyle}><div><strong>{total.toLocaleString()}</strong><span>Total Vendors</span></div></div><ul><li><i className="g"/>Portal Enabled <b>{enabled.toLocaleString()} ({pct(enabled,total)})</b></li><li><i className="gray"/>Not Enabled <b>{notEnabled.toLocaleString()} ({pct(notEnabled,total)})</b></li></ul></div></div><div className="role-panel"><h3>PO &amp; GRN Trend (Last 6 Months)</h3><TrendBars rows={data.trend||[]} leftLabel="Purchase Orders" rightLabel="GRNs"/></div><VendorBars title="Top Vendors (by PO Amount)" rows={data.topVendors||[]}/></section>
    <section className="role-table-card"><div className="role-tabs"><button className={tab==="po"?"active":""} onClick={()=>setTab("po")}>Recent Purchase Orders</button><button className={tab==="grn"?"active":""} onClick={()=>setTab("grn")}>Recent GRNs</button><button className={tab==="pending"?"active":""} onClick={()=>setTab("pending")}>Pending Vendor Requests</button><button className={tab==="onboarded"?"active":""} onClick={()=>setTab("onboarded")}>Recently Onboarded Vendors</button><button className="role-export" onClick={exportRows}>↧ &nbsp; Export to Excel</button></div>
      <div className="role-filters"><label><span>From Date</span><input type="date" value={from} onChange={e=>setFrom(e.target.value)}/></label><label><span>To Date</span><input type="date" value={to} onChange={e=>setTo(e.target.value)}/></label><label><span>Vendor</span><select value={vendor} onChange={e=>setVendor(e.target.value)}><option value="">All Vendors</option>{vendors.map(v=><option key={v}>{v}</option>)}</select></label><label><span>Status</span><select value={status} onChange={e=>setStatus(e.target.value)}><option value="">All Statuses</option>{statuses.map(s=><option key={s}>{s}</option>)}</select></label><label className="role-search-box"><span>&nbsp;</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search PO number, vendor, item..."/></label><button className="role-search" onClick={()=>setApplied({from,to,vendor,status,search})}>⌕ &nbsp; Search</button><button className="role-clear" onClick={()=>{setFrom("");setTo("");setVendor("");setStatus("");setSearch("");setApplied({from:"",to:"",vendor:"",status:"",search:""});}}>Clear</button></div>
      <div className="role-table-scroll">{tab==="po"?<table className="role-table"><thead><tr><th>PO Number</th><th>Vendor</th><th>PO Date</th><th>Amount (PKR)</th><th>Status</th><th>GRN Status</th><th>Invoice Status</th><th>Actions</th></tr></thead><tbody>{visible.map((r:any)=><tr key={r.id}><td>{r.poNumber}</td><td>{r.vendorName}</td><td>{formatDate(r.poDate)}</td><td>{formatMoney(r.amount)}</td><td><StatusPill value={r.status}/></td><td><StatusPill value={r.grnStatus}/></td><td><StatusPill value={r.invoiceStatus}/></td><td><span className="role-actions">⌾ &nbsp; ▱</span></td></tr>)}{!visible.length&&<tr><td colSpan={8} className="empty">No live records match the selected filters.</td></tr>}</tbody></table>:tab==="grn"?<table className="role-table"><thead><tr><th>GRN Number</th><th>Vendor</th><th>PO Number</th><th>GRN Date</th><th>Status</th><th>QC Status</th><th>Actions</th></tr></thead><tbody>{visible.map((r:any)=><tr key={r.id}><td>{r.grnNumber}</td><td>{r.vendorName}</td><td>{r.poNumber}</td><td>{formatDate(r.grnDate)}</td><td><StatusPill value={r.status}/></td><td>{r.qcStatus||"-"}</td><td><span className="role-actions">⌾ &nbsp; ▱</span></td></tr>)}</tbody></table>:<table className="role-table"><thead><tr><th>Vendor</th><th>Oracle Vendor ID</th><th>Status</th><th>Portal Users</th><th>Last Access / Onboarded</th></tr></thead><tbody>{visible.map((r:any,i:number)=><tr key={r.vendorId||`${r.vendorName}-${i}`}><td>{r.vendorName}</td><td>{r.oracleVendorId||"-"}</td><td><StatusPill value={r.accessStatus||(r.active?"Active":"Inactive")}/></td><td>{r.portalUsers??r.activePortalUsers??1}</td><td>{formatDate(r.lastAccess||r.onboardedAt)}</td></tr>)}{!visible.length&&<tr><td colSpan={5} className="empty">No live records match the selected filters.</td></tr>}</tbody></table>}</div><Pager page={safePage} pages={pages} total={filtered.length} pageSize={pageSize} onPage={setPage}/><div className="role-info">ⓘ &nbsp; You are logged in as <strong>Supply Chain</strong>. Dashboard values are loaded from live portal data.</div>
    </section></div>;
}
