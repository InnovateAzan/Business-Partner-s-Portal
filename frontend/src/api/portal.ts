...
import type { OracleInvoice, OraclePoGrn, OracleSupplier, PortalInvoice, OracleReceiptLine } from "../types";
...
export async function resubmitInvoice(id:string,payload:FormData,idempotencyKey?:string){ const {data}=await api.post(`/invoices//resubmit`,payload,{headers:{"Content-Type":"multipart/form-data",...(idempotencyKey?{"Idempotency-Key":idempotencyKey}:{})}}); return data; }
...
export async function getMyReceiptLines(poNumber:string,grnNumbers:string[]=[]){ const {data}=await api.get<OracleReceiptLine[]>("/oracle/receipt-lines/my",{params:{poNumber,grnNumbers:grnNumbers.join(",")}}); return data; }
